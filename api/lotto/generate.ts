...
const ELEMENT_MESSAGES: Record<string, string[]> = {
  "목(木)": ["성장과 발전의 기운을 가진 목(木)의 에너지로 선택된 번호예요!", "봄의 생명력처럼 번창할 행운의 번호들입니다!"],
  "화(火)": ["열정과 창의력의 화(火) 기운이 당신의 행운번호를 밝혀줍니다!", "뜨거운 열정으로 행운을 잡아보세요!"],
  "토(土)": ["안정과 균형의 토(土) 기운이 깃든 행운의 번호들이에요!", "대지의 풍요로운 기운을 담은 번호들입니다!"],
  "금(金)": ["풍요와 성공의 금(金) 기운이 당신을 행운으로 이끌어요!", "황금빛 행운이 이 번호들과 함께해요!"],
  "수(水)": ["지혜와 유연함의 수(水) 기운이 담긴 행운번호예요!", "흐르는 물처럼 행운이 자연스럽게 찾아올 거예요!"],
};
...
function generateNumbers(year: number, month: number, day: number, hour: number): number[] {
  ...
  while (baseNumbers.size < 6) baseNumbers.add(Math.floor(rng() * 45) + 1);
...
function generateBonusNumber(numbers: number[], seed: number): number {
  ...
  let bonus: number;
  do { bonus = Math.floor(rng() * 45) + 1; } while (numbers.includes(bonus));
  return bonus;
}
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.method !== "POST") {
    res.writeHead(405);
    res.end(JSON.stringify({ error: "METHOD_NOT_ALLOWED", message: "POST만 허용됩니다." }));
    return;
  }
  ...
  try {
    ...
    res.writeHead(400);
    res.end(JSON.stringify({ error: "INVALID_JSON", message: "요청 형식이 잘못됐습니다." }));
    return;
  }
  ...
  if (
    ...
    year < 1900 || year > 2006 || month < 1 || month > 12 ||
    day < 1 || day > 31 || hour < 0 || hour > 23
  ) {
    res.writeHead(400);
    res.end(JSON.stringify({ error: "INVALID_INPUT", message: "올바른 생년월일시를 입력해주세요." }));
    return;
  }
  ...
  const birthElement = ELEMENTS_KOREAN[Math.floor(heavenlyStemIndex / 2)];
  const zodiacSign = `${ZODIAC_SIGNS[earthlyBranchIndex]}띠`;
  const fortuneScore = 50 + ((lifeNumber * 8 + heavenlyStemIndex * 6 + earthlyBranchIndex * 4 + (hour % 12) * 5) % 50);
  ...
  const msgPool = [...FORTUNE_MESSAGES, ...(ELEMENT_MESSAGES[fortuneElement] ?? [])];
